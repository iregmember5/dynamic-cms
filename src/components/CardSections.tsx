...
const CardSections: React.FC<CardSectionsProps> = ({ data }) => {
  ...
  if (!card_sections || !card_sections.cards || card_sections.cards.length === 0) return null;
  ...
  const renderCard = (card: CardContent, index: number) => {
    ...
    return (
      <div
        key={card.id}
        className={`${cardStyles[card.card_style] || cardStyles.basic} p-6 sm:p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group`}
        style={{
          animationDelay: `${index * 0.1}s`,
          borderColor: card.card_style === 'pricing' ? `${primaryColor}40` : 'transparent'
        }}
      >
        {/* Image */}
        {card.image && (
        ...
        {card.button_text && card.button_url && (
          <a
            ...
            style={{
              backgroundColor: card.card_style === 'pricing' ? primaryColor : `${primaryColor}15`,
